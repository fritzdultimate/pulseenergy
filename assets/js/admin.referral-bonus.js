...
let actionMsgs = {
    fund : {
        title : 'Fund this user\'s referral bonus?',
    },
    debit : {
        title : 'Debit this user\'s referral bonus?',
    },
}
...
window.addEventListener('load', () => {
    ...
    $('#user-list').on('select2:select', function (e) {
        ...
        blockUI('<h5>Fetching Referral Bonus...</h5>');
        getUserData(data.id);
    });
...
function getUserData(user){
    ...
    .then((data) => {
      userBalance.textContent = '$' + data['referral_bonus'];
     }).catch((err) => {
        LobiNotify('error', catchErrorMsg);
     });
}
function manageWallet(form){
    fetch(urlPrefix + 'account/' + form['action'] + '-referral-bonus', {
        method : 'post',
        headers,
